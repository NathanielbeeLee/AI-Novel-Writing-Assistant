...
### 2026-07-16（整书质量闭环与统一资产中心）

小说工作台增加面向长篇一致性的整书质量工具：角色可按全书重要度分级，全书审校建议经确认后可持续约束后续章节，并可对已有章节批量润色。

- 角色支持“核心主角、主要角色、具名配角、临时角色”四档，生成上下文会优先保留更重要角色的状态与硬事实。
- 全书级审校跨章节检查连续性、角色弧、情节因果、节奏、文风和伏笔回收，并为问题提供章节证据。
- 审校建议默认不会污染正文；只有点击采用后才回灌为后续章节可读取的创作约束。
- 批量润色复用现有快照、质检、轻修和恢复链路，可处理已经有正文的章节范围。
- 新增统一资产中心，从一个入口访问题材、推进模式、知识、世界、写法、角色、标题和拆书资产。
- 本次包含数据库增量迁移；桌面端启动时自动处理，源码部署请按现有迁移流程更新数据库。
