...
- 自动探测只有在普通与结构化调用使用同一协议都成功时才会采用，避免测试结果与正式写作错位。
- 新增[项目总手册](./docs/wiki/project-handbook.md)与[模型协议 Wiki](./docs/wiki/architecture/llm-request-protocols.md)，集中说明章节定点修改、伏笔、批量长篇、CLIProxyAPI 配置和排障边界。
