...
- 自动导演提供三种主要运行方式：**先准备到可开写**（推荐第一本书）、**全书自动成书**、**按范围执行**（全书 / 前 N 章 / 第 1 卷）；**正文后去 AI 检测与修正**是可叠加的质量闭环
...
### 2026-07-16（自定义模型接入与整书质量闭环）

长篇创作链路同时补齐模型接入、世界观准备、角色持续追踪和跨章节质量控制；可以先用自己的网关与模型完成设定，再按章节范围自动生成、审校和润色。

- 自定义厂商可以填写 Base URL、API Key 和模型名称，自动读取模型列表，并明确选择 Responses API 或 Chat Completions。
- OpenAI-compatible Base URL 可填写域名根地址或 `/v1` 地址；根路径端点返回 404/405 时会安全尝试 `/v1`，不提供模型目录的网关仍可保留手填模型。
- Responses 流现在兼容部分第三方网关省略最终 `output` 或文本注解的缩减事件，连接测试和实际生成不会再因此在消息组装阶段异常退出。
- 世界观准备会在角色与剧情规划前整理本书规则、地点和势力；自动导演恢复时会保留已有规划与正文，并引导补齐缺失资产。
- 角色支持“核心主角、主要角色、具名配角、临时角色”四档，生成上下文会优先保留重要角色的状态与硬事实。
- 全书级审校会结合章节证据检查连续性、角色弧、因果、节奏、文风与伏笔；只有确认采用的建议才会回灌后续章节。
- 批量润色复用章节快照、质检、轻修和恢复链路，可按范围处理已有正文，不因局部质量问题中断整本推进。
- AI 实况、运行记录和新版角色工作台用于查看生成过程、恢复位置、角色关系、思路线、对话与形象资料。
- 统一资产中心集中访问题材、推进模式、知识、世界、写法、角色、标题与拆书资产，视觉资源仍可在专门的资源库筛选和复用。
- 桌面端会在启动时应用新增数据库字段；源码部署请按现有迁移流程更新数据库。
...
更细的文档分区说明可以看 [docs/README.md](./docs/README.md)；要快速理解产品能力、使用路径和代码入口，先读[项目总手册](./docs/wiki/project-handbook.md)。
