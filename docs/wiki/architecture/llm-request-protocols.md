...
### Responses 兼容网关的流事件归一化

OpenAI 官方 Responses 流在 `response.completed` 的 `response.output` 中带回完整 output item，文本块也包含 `annotations` 数组。部分 OpenAI-compatible 网关只转发 `response.output_text.delta`，完成事件省略 `output`，或者省略文本块的 `annotations`。这类响应有实际正文，但会让上层 SDK 在组装最终消息时因为对缺失数组调用 `.map()` 而失败。

本项目在 Responses 协议适配器边界做最小归一化：

- 累积流中的文本 delta；
- 完成事件缺少 `output` 时，用已接收文本补成标准 assistant message；
- output text 缩减了 `annotations` 时补为空数组；
- 官方完整 payload 保持原有字段，不把显式 Responses 请求静默改发 Chat Completions。

归一化只修复响应外形，不会伪造被网关完全丢失的工具调用、推理信息或用量数据。如果网关没有发送文本 delta、最终文本或完整 output，调用仍应按失败处理。
...
- 如果日志显示 `requestProtocol=openai_responses`，旧版本可能收到了缺少 `output` 或 `annotations` 的兼容网关事件；更新到包含 Responses 流归一化的版本后重试。
- 更新后仍报错时，保留 `requestId`、事件类型、厂商、模型和协议日志，但必须删除 API Key；不要把真实 Key 发到聊天、Issue 或 PR。
- 如果错误改为 404、401 或 429，应分别检查端点、凭据或额度，它们不是同一种 payload 兼容问题。
...
- Responses 缩减流回归测试：`server/tests/responsesStreamCompatibility.test.js`
