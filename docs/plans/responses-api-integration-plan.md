...
- `both` 探针只有在普通与结构化调用使用同一个协议都成功时才算完整可用，不能拼接两个端点的部分成功。
...
- [x] 厂商可保存 BaseURL、Key、模型名与默认请求协议。
- [x] 可从 CLIProxyAPI `/v1/models` 自动读取模型，失败时可手动填写。
- [x] Responses 普通、流式、结构化请求走统一适配，并发送 `store: false`。
- [x] 显式 Chat Completions 通过协议绑定客户端禁止 LangChain 自动切换。
- [x] 任务路由可继承厂商协议，也可按任务覆盖。
- [x] 旧数据通过默认值 `auto` 保持 Chat Completions 行为。
- [x] 测试可直接断言实际 HTTP 路径和关键请求字段。
- [x] 整体项目手册、协议 Wiki、用户配置文档和发布说明均有入口。

## 7. 验证状态

已完成与本功能直接相关的验证：

- `pnpm check:docs-manifest`：通过，33 篇公开文档与 17 个导演进度键均已登记。
- shared、server build 与 client typecheck：通过。
- `pnpm build`：通过；Vite 仅报告已有的大 chunk 提示。
- desktop typecheck：通过。
- `server/tests/llmRequestProtocols.test.js`：6/6 通过，覆盖协议优先级、厂商继承、CLIProxyAPI 风格 `/v1/models`、同协议双探针、Responses 普通/结构化/流式/usage/`store:false`、强制 Chat。
- `server/tests/modelRouter.test.js`：9/9 通过。
- 协议选择前端纯函数测试：2/2 通过。
- 自定义厂商设置路由目标测试：1/1 通过。
- `git diff --check`：通过。

未把仓库全量测试表述为全绿：

- 服务端 fast 聚合集在执行完主体后未自行退出，已中断；从输出抽取的 6 个未改动测试文件单独执行为 49/59 通过、10 项失败，涉及自动审批审计旧断言、缺少本地迁移表的拆书测试、拆书角色夹具、章节义务旧预期、章节上下文旧预期与工具 schema 守卫，均不在本功能改动路径。
- 客户端全量测试当前有 5 项既有失败：Node 环境读取 `import.meta.env`，以及移动端新增 drama 路由后旧路由/CSS契约未更新；本功能新增的协议测试单独通过。
- 当前容器没有用户本机 CLIProxyAPI 和真实 Key，不能代替用户完成真实网关/真实模型/UI 点击验收。合并前应按协议 Wiki 的 CLIProxyAPI 七步流程做一次短章 smoke test。
