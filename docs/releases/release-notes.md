Warning: truncated output (original token count: 64801)
Total output lines: 1736
...
- 当前模型选择会保存到服务端设置中，换浏览器缓存或桌面启动环境后…4801 tokens truncated…看到每个提示词的上下文需求、可安全调整的表达槽位、锁定字段、结构化输出和重试 / 修复能力；也可以按小说、章节或任务读取当前提示词需要的资料，直接看到哪些资料已准备好、哪些缺输入、哪些被裁剪，方便先排查资料装配问题，再决定是否进入后续优化。
