...
- 角色支持“核心主角、主要角色、具名配角、临时角色”四档。AI 生成阵容时会给出分级，用户也可以在角色编辑区调整；章节上下文优先排列重要角色，减少长篇中核心人物被边缘资料挤出的问题。
- 全书级审校会在指定章节范围内检查连续性、角色弧、情节因果、节奏、文风和伏笔回收。报告包含评分、优点、问题章节和证据，避免只给笼统评价。
- 跨章节回灌改为明确确认：审校结果先保存为报告，只有点击“采用并回灌”后才转成后续章节可读取的创作约束；重复采用同一问题不会产生重复约束。
- 批量润色提供单独入口，默认对已有正文开启逐章审核与轻量修复，并复用现有快照、任务恢复和质量债务机制。
- 新增统一资产中心，从一个入口访问题材、推进模式、知识文档、世界样本、写法、基础角色、标题和拆书资产；具体编辑仍在成熟的专项工作台完成。
- SQLite 与 PostgreSQL 都包含角色重要度增量迁移。桌面端会在启动流程补齐字段；源码部署用户需要按现有数据库迁移流程更新。
...
- 厂商配置支持自定义 Base URL、API Key、默认模型和请求协议；模型列表仍可通过标准 `/v1/models` 接口自动获取。
- Responses API 覆盖普通生成、JSON Schema 结构化输出、流式交互和 token 用量统计，并默认发送 `store: false`。
- 模型路由的“自动”会继承厂商协议；单个路由或连接测试可以显式覆盖，显式选择时不会在失败后静默改用另一种协议。
- Chat Completions 仍保持兼容；即使模型名被底层 SDK 判断为更适合 Responses，显式选择 Chat 时也会固定调用 `/v1/chat/completions`。
- 自动连接测试现在要求普通调用与结构化调用在同一个协议上都成功，避免把两个端点的部分成功拼成完整兼容；厂商弹窗会选中检测到的协议，用户保存后生效。
- SQLite 与 PostgreSQL 都包含兼容的增量迁移，既有厂商默认保持“自动”，升级后无需重填 Key 和模型。
- 新增项目总手册和模型协议 Wiki，集中说明创意/世界/角色协作、指定章节修改、伏笔账本、批量长篇、CLIProxyAPI 配置、代码地图与排障边界。
