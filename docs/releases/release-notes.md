...
- 自定义模型厂商可以填写 Base URL、API Key、默认模型和请求协议，适合接入 CLIProxyAPI 以及提供 OpenAI 兼容接口的 OpenAI、Qwen、Kimi 等模型渠道。
- OpenAI-compatible Base URL 支持填写域名根地址或带 `/v1` 的地址；仅在端点明确返回 404/405 时安全尝试 `/v1`，不会因认证、限流或服务器错误重复生成请求。
- 厂商配置可以通过标准模型目录自动读取可用模型；网关不提供模型目录时，仍可直接填写模型名称。
- 模型目录会同时兼容根路径与 `/v1/models`；网关完全不提供目录接口时，刷新操作会保留手填模型并给出说明。
- Responses API 支持普通生成、结构化输出、流式交互和用量统计，并默认关闭远端响应存储；Chat Completions 与 Anthropic 兼容调用继续保留。
- Responses 流兼容部分第三方网关省略最终 `output` 或文本注解的缩减事件；已收到的正文会被安全组装为标准消息，连接测试与实际生成不再因此异常退出。
- 模型路由可以继承厂商协议，也可以按具体任务覆盖。明确选择 Responses API 或 Chat Completions 后，失败时不会悄悄切换到另一种协议。
- 自动连接测试会要求普通调用和结构化调用在同一种协议下都成功，避免把两个端点各自的部分成功误判为完整可用。
- 角色可以标记为“核心主角、主要角色、具名配角、临时角色”。AI 会优先把重要角色的状态和硬事实带入章节生成，降低长篇后段主角信息被次要角色挤出的风险。
- 全书级审校可以检查指定章节范围的连续性、角色弧、情节因果、节奏、文风和伏笔回收，并给出可定位到章节的证据，而不是只返回笼统评价。
- 全书审校建议会先保存为报告；只有用户确认采用的问题才会成为后续章节可读取的创作约束，重复采用不会生成重复约束。
- 批量润色可以按范围处理已有正文，沿用章节快照、逐章质检、轻量修复、恢复点和质量提醒；可用正文不会因为局部问题而阻断剩余章节。
- 统一资产中心提供题材、推进模式、知识、世界、写法、基础角色、标题和拆书资产的集中入口；具体编辑继续在各自专项工作台完成，视觉资源可在视觉资源库中筛选与复用。
- 新增能力已融合世界观准备、AI 实况、运行记录、新版角色工作台、读者体验合同和承诺账本等主线能力；自动导演仍按世界观、角色、卷章规划、正文、审校和回灌的顺序推进。
- SQLite 与 PostgreSQL 都包含兼容的增量迁移。桌面端会在启动时补齐字段；源码部署用户需要按现有迁移流程更新数据库。
